...
function iterator( k, lambda, options ) {
	...
	if ( !isPositiveInteger( k ) ) {
		throw new TypeError( format( '0pb4b,Ht', k ) );
	}
	if ( !isPositiveNumber( lambda ) ) {
		throw new TypeError( format( '0pb72,Go', lambda ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0pb2V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0pb2t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
