...
function iterator( k, lambda, options ) {
	...
	if ( !isPositiveInteger( k ) ) {
		throw new TypeError( format( '0Ph4o', k ) );
	}
	if ( !isPositiveNumber( lambda ) ) {
		throw new TypeError( format( '0Ph7T', lambda ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Ph2h', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Ph35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
